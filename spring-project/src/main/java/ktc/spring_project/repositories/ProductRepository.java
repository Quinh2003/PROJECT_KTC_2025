...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    @Query("SELECT p FROM Product p WHERE p.productStatus = ktc.spring_project.enums.ProductStatus.AVAILABLE ORDER BY p.name")
    List<Product> findActiveProductsOrderByName();

    @Query("SELECT p FROM Product p WHERE p.stockQuantity <= :threshold AND p.productStatus = ktc.spring_project.enums.ProductStatus.AVAILABLE")
    List<Product> findProductsWithLowStock(@Param("threshold") Integer threshold);

    @Query("SELECT p FROM Product p WHERE LOWER(p.name) LIKE LOWER(CONCAT('%', :name, '%')) AND p.productStatus = ktc.spring_project.enums.ProductStatus.AVAILABLE")
...
List<Product> findActiveByCategoryId(@Param("categoryId") Long categoryId);
    ...
    @Query("SELECT SUM(p.stockQuantity) FROM Product p WHERE p.productStatus = ktc.spring_project.enums.ProductStatus.AVAILABLE")
