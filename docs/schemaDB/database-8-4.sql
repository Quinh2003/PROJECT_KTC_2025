-- Đã loại bỏ tất cả các bảng, index, view, liên kết liên quan đến các bảng có từ "backup".
-- Các phần giữ lại là: addresses, categories, deliveries, delivery_proofs, delivery_tracking, orders, order_items, payments, products, roles, routes, status, stores, users, warehouses, warehouse_transactions, activity_logs, order_checklist, electronic_invoices, các view và index liên quan (không chứa backup).
...
alter table deliveries
	add constraint fk_deliveries_tracking_id foreign key (tracking_id) references delivery_tracking (id);
...
create index idx_delivery_tracking_vehicle_time on delivery_tracking (vehicle_id asc, timestamp desc) comment 'Tracking theo thời gian mới nhất';
...
(
	...
	constraint FKov6eiiyhqkasfj9jlt2s6wj5p foreign key (vehicle_id) references vehicles (id),
...
create index idx_warehouse_trans_product_date on warehouse_transactions (product_id asc, transaction_date desc) comment 'Lịch sử giao dịch sản phẩm';
...
(
	...
	constraint warehouse_code unique (warehouse_code),
...
(
	...
	constraint order_checklist_ibfk_1 foreign key (order_id) references orders (id),
	constraint order_checklist_ibfk_2 foreign key (updated_by) references users (id)
);

create index order_id on order_checklist (order_id);
create index updated_by on order_checklist (updated_by);
...
(
	...
	delivery_id bigint not null,
	order_id bigint not null,
	constraint UKa55wbx1mm1369td5h24ku1q7y unique (invoice_number),
	constraint FK4kna1tpx41rx2f4inn3w9hpqk foreign key (delivery_id) references deliveries (id),
	constraint FK8yu3647a19pwb3ub2u2dcawvj foreign key (created_by) references users (id),
	constraint FKgj4n91bvmarpys9thb5624xi2 foreign key (order_id) references orders (id),
	constraint FKl5lqu45tlobg150wdhpcfu7fo foreign key (cancelled_by) references users (id)
);

-- Các view giữ nguyên, không chứa backup
-- ... (giữ lại toàn bộ phần định nghĩa view như script gốc, chỉ bỏ các view liên quan đến bảng backup nếu có, tuy nhiên script gốc không có view dùng bảng backup)
