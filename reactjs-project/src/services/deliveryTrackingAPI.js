 ...
 */
export async function createDeliveryTrackingPoint(data) {
    const token = localStorage.getItem("token");
    // Helper function to format date for backend
    const formatTimestamp = (date) => {
        return date.toISOString().replace('T', ' ').replace('Z', '').substring(0, 19);
 ...
 */
async function getDeliveryByOrderIdFallback(orderId) {
    const token = localStorage.getItem("token");
    const response = await fetch(`${API_BASE_URL}/api/deliveries`, {
        method: 'GET',
        headers: {
 ...
 */
export async function getDeliveryTrackingPoints(deliveryId) {
    const token = localStorage.getItem("token");
 ...
 */
export async function getDeliveriesWithPagination(page = 0, size = 20, orderId) {
    const token = localStorage.getItem("token");
 ...
 */
export async function getStartCoordinatesFromStore(storeId) {
    try {
 ...
 */
export function getDeliveryStatusId(statusName) {
    const statusMap = {
...
export async function updateDelivery(deliveryId, data) {
    ...
    if (data.transportMode)
        updateData.transportMode = data.transportMode;
    if (data.serviceType)
        updateData.serviceType = data.serviceType;
    if (data.scheduleDeliveryTime)
        updateData.scheduleDeliveryTime = formatTimestamp(new Date(data.scheduleDeliveryTime));
