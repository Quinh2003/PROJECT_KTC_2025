...
export async function fetchOrdersTotalQuantityBatch(orderIds: (number | string)[]): Promise<{ [orderId: string]: number }> {
  ...
  if (!res.ok) throw new Error("Failed to fetch batch total quantity");
  return res.json();
}
// Lấy danh sách sản phẩm của 1 đơn hàng (có phân trang)
export async function fetchOrderItemsByOrderIdPaged(orderId: number | string, page = 0, size = 10): Promise<{ content: ProductItem[]; totalElements: number; totalPages: number; }> {
  const token = localStorage.getItem("token");
  const res = await fetch(`http://localhost:8080/api/order-items/order/${orderId}/paged?page=${page}&size=${size}`, {
    headers: token ? { "Authorization": `Bearer ${token}` } : undefined,
  });
  if (!res.ok) throw new Error("Failed to fetch paged order items");
...
export async function fetchAllOrderItemsPaged(page = 0, size = 10): Promise<{ content: ProductItem[]; totalElements: number; totalPages: number; }> {
  const token = localStorage.getItem("token");
  const res = await fetch(`http://localhost:8080/api/order-items?page=${page}&size=${size}`, {
    headers: token ? { "Authorization": `Bearer ${token}` } : undefined,
  });
  if (!res.ok) throw new Error("Failed to fetch all order items paged");
  const data = await res.json();
  return {
    content: Array.isArray(data.content)
      ? data.content.map((item: any) => ({
...
export async function fetchOrderItemsByOrderId(orderId: number | string): Promise<ProductItem[]> {
  const token = localStorage.getItem("token");
