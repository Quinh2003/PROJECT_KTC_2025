...
export async function getRoute(start, waypoints, end, token) {
    ...
    if (!res.ok)
        throw new Error('Không lấy được lộ trình');
    return res.json();
}
export async function postRoute(route, waypoints) {
    ...
    if (!res.ok)
        throw new Error('Không gửi được lộ trình');
    return res.json();
}
export async function postTruckPosition(position) {
    ...
    if (!res.ok)
        throw new Error('Không gửi được vị trí xe');
    return res.json();
}
